<template>
  <view class="uv-icon" @tap="clickHandler" :class="['uv-icon--' + labelPos]">
    <image
      class="uv-icon__img"
      v-if="isImg"
      :src="name"
...
export default {
  ...
  computed: {
    ...
    isImg() {
      const isBase64 =
        this.name.indexOf('data:') > -1 && this.name.indexOf('base64') > -1;
    ...
    imgStyle() {
      ...
      style.width = this.width
        ? this.$uv.addUnit(this.width)
        : this.$uv.addUnit(this.size);
      style.height = this.height
        ? this.$uv.addUnit(this.height)
        : this.$uv.addUnit(this.size);
      return style;
    },
    // 通过图标名，查找对应的图标
    icon() {
      ...
      if (!code) {
        return code
          ? unescape(`%u${code}`)
          : ['uvicon'].indexOf(this.customPrefix) > -1
          ? unescape(`%u${this.name}`)
          : '';
      }
      // #endif
      return code
...
@import '../../../uv-ui-tools/libs/css/components.scss';
@import '../../../uv-ui-tools/libs/css/color.scss';
// 变量定义
