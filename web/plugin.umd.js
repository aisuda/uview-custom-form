(function(global2, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory(require("react"), require("vue")) : typeof define === "function" && define.amd ? define(["react", "vue"], factory) : (global2 = typeof globalThis !== "undefined" ? globalThis : global2 || self, global2.plugin = factory(global2.react, global2.vue));
  ...
  function compile(tokens, values) {
    const compiled = [];
    let index = 0;
    const mode = isArray$7(values) ? "list" : isObject$4(values) ? "named" : "unknown";
    if (mode === "unknown") {
      return compiled;
    }
  ...
  function getRouteOptions(path, alias = false) {
    if (alias) {
      return __uniRoutes.find((route2) => route2.path === path || route2.alias === path);
    }
  ...
  function hasCallback(args) {
    if (isPlainObject(args) && [API_SUCCESS, API_FAIL, API_COMPLETE].find((cb) => isFunction$2(args[cb]))) {
      return true;
    }
    return false;
  }
  ...
  const upx2px = /* @__PURE__ */ defineSyncApi(API_UPX2PX, (number2, newDeviceWidth) => {
    if (deviceWidth === 0) {
      checkDeviceWidth();
      {
        checkMaxWidth();
  ...
  function createRootApp(component, rootState, callback) {
    ...
    return vue.createApp(vue.defineComponent({
      setup() {
        return () => (vue.openBlock(), vue.createBlock(component, rootState, null, 16));
      }
    }));
  }
  ...
  function navigate({ type, url: url2, events }, __id__) {
  ...
  const navigateTo = /* @__PURE__ */ defineAsyncApi(API_NAVIGATE_TO, ({ url: url2, events }, { resolve, reject }) => navigate({ type: API_NAVIGATE_TO, url: url2, events }).then(resolve).catch(reject), NavigateToProtocol, NavigateToOptions);
  ...
  const redirectTo = /* @__PURE__ */ defineAsyncApi(API_REDIRECT_TO, ({ url: url2 }, { resolve, reject }) => {
    return removeLastPage(), navigate({ type: API_REDIRECT_TO, url: url2 }).then(resolve).catch(reject);
  ...
  const reLaunch = /* @__PURE__ */ defineAsyncApi(API_RE_LAUNCH, ({ url: url2 }, { resolve, reject }) => {
  ...
  const switchTab = /* @__PURE__ */ defineAsyncApi(API_SWITCH_TAB, ({ url: url2 }, { resolve, reject }) => {
    return removeNonTabBarPages(), navigate({ type: API_SWITCH_TAB, url: url2 }, getTabBarPageId(url2)).then(resolve).catch(reject);
  ...
  function useToastIcon(props2) {
    const Icon = vue.computed(() => {
      switch (props2.icon) {
        case "success":
          return vue.createVNode(createSvgIconVNode(ICON_PATH_SUCCESS_NO_CIRCLE, "#fff", 38), {
            class: ToastIconClassName
          });
        case "error":
          return vue.createVNode(createSvgIconVNode(ICON_PATH_WARN, "#fff", 38), {
            class: ToastIconClassName
          });
        case "loading":
  ...
  function watchVisible() {
    scope.run(() => {
      vue.watch([() => showToastState.visible, () => showToastState.duration], ([visible, duration]) => {
        if (visible) {
  ...
  */
  ...
  function AddCustomPlugin(id2, plugin) {
    ...
    if (!window.AIPageEditorCustomPlugins[componentId]) {
      ...
      console.error(`${consoleTag}\u6CE8\u518C\u81EA\u5B9A\u4E49\u63D2\u4EF6\u5931\u8D25\uFF0C\u5DF2\u5B58\u5728\u540C\u540D\u63D2\u4EF6(${id2})\u3002`);
    }
    return null;
  }
